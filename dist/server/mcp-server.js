...
// ── get_project ──────────────────────────────────────────────────────────────
server.tool("get_project", "Get a project by ID (includes categories, stages, task summary)", { project_id: z.string().describe("Project ID") }, async ({ project_id }) => {
    const db = readDB();
    ...
    const summary = {
        ...
        tasksByStage: project.stages.reduce((acc, s) => {
...
// ── list_tasks ───────────────────────────────────────────────────────────────
server.tool("list_tasks", "List tasks for a project, optionally filtered by stage or category", {
    ...
    if (category_id)
        tasks = tasks.filter((t) => t.categoryId === category_id);
...
// ── update_task ───────────────────────────────────────────────────────────────
server.tool("update_task", "Update a task's fields (title, description, stage, category, priority, assignee, linked doc)", {
    project_id: z.string().describe("Project ID"),
    task_id: z.string().describe("Task ID"),
    ...
    if (linked_doc_file_id !== undefined)
        task.linkedDocFileId = linked_doc_file_id || undefined;
    task.updatedAt = now();
    writeDB(db);
    return { content: [{ type: "text", text: `Task updated:\n${JSON.stringify(task, null, 2)}` }] };
});
// ── delete_task ───────────────────────────────────────────────────────────────
server.tool("delete_task", "Delete a task from a project", {
    project_id: z.string().describe("Project ID"),
    task_id: z.string().describe("Task ID"),
}, async ({ project_id, task_id }) => {
    const db = readDB();
...
// ── list_categories ───────────────────────────────────────────────────────────
server.tool("list_categories", "List all categories/disciplines for a project", { project_id: z.string().describe("Project ID") }, async ({ project_id }) => {
    const db = readDB();
    const project = db.projects.find((p) => p.id === project_id);
    if (!project)
        return { content: [{ type: "text", text: `Project "${project_id}" not found.` }] };
...
// ── get_design_doc ────────────────────────────────────────────────────────────
server.tool("get_design_doc", "Get the design document (markdown) for a project", { project_id: z.string().describe("Project ID") }, async ({ project_id }) => {
    const db = readDB();
    const project = db.projects.find((p) => p.id === project_id);
    if (!project)
        return { content: [{ type: "text", text: `Project "${project_id}" not found.` }] };
...
// ── update_design_doc ─────────────────────────────────────────────────────────
server.tool("update_design_doc", "Replace the design document content for a project", {
    project_id: z.string().describe("Project ID"),
